...
let sexp_of_content content =
  ...
  | `Empty ->
    sexp_of_string ""
  | `String s ->
    sexp_of_string (escape_html s)
  | `Bigstring b ->
    sexp_of_string (escape_html (Bigstringaf.to_string b))
  | `Stream s ->
    sexp_of_opaque s
...
let to_string { content; _ } =
  ...
  | `String s ->
    Lwt.return s
  | `Bigstring b ->
    Lwt.return (Bigstringaf.to_string b)
  | `Empty ->
    Lwt.return ""
...
let to_stream { content; _ } =
  ...
  | `Bigstring b ->
    Lwt_stream.of_list [ Bigstringaf.to_string b ]
  | `Stream s ->
    s
...
let of_bigstring b =
  { content = `Bigstring b; length = len (Bigstringaf.length b) }
...
let copy t =
  ...
  | `Bigstring _ ->
    t
  | `Stream stream ->
    { t with content = `Stream (Lwt_stream.clone stream) }
