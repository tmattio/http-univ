...
module Route = struct
  ...
  let rec sexp_of_t (t : t) : Sexp.t =
    ...
    | Nil ->
      Sexp.Atom "Nil"
    | Full_splat ->
      Sexp.Atom "Full_splat"
    | Literal (x, y) ->
      Sexp.List [ Sexp.Atom x; sexp_of_t y ]
    | Param (x, y) ->
      let x : Sexp.t =
        match x with Some x -> Sexp.Atom (":" ^ x) | None -> Sexp.Atom "*"
      in
      Sexp.List [ x; sexp_of_t y ]
  ...
  let rec parse_tokens params tokens =
    ...
    | token :: tokens ->
      if token = "" then
        raise (E "Double '/' not allowed")
      else if token = "*" then
        Param (None, parse_tokens params tokens)
      else if token = "**" then
        raise (E (sprintf "double splat allowed only in the end"))
      else if token.[0] = ':' then
        let name =
          let len = String.length token in
          if len > 1 then
            String.sub token 1 (len - 1)
          else
            raise (E "Named paramter is missing a name")
        in
        let params =
          if List.mem name params then
            raise (E (sprintf "duplicate parameter %S" name))
          else
            name :: params
        in
        Param (Some name, parse_tokens params tokens)
      else
        Literal (token, parse_tokens params tokens)
  ...
  let of_string s =
    ...
    | "" :: tokens ->
      parse_tokens [] tokens
    | _ ->
      raise (E "route must start with /")

  let of_string_result s =
    match of_string s with exception E s -> Error s | s -> Ok s
end
...
module Params = struct
  ...
  let splat t =
    ...
    | None ->
      t.unnamed
    | Some r ->
      t.unnamed @ String.split_on_char '/' r
...
let sexp_of_smap f smap : Sexp.t =
  let open Sexp in
  List (Smap.bindings smap |> List.map (fun (k, v) -> Sexp.List [ Atom k; f v ]))

let rec sexp_of_t f t =
  let open Sexp_conv in
  match t with
  | Accept (a, r) ->
    (sexp_of_pair f Route.sexp_of_t) (a, r)
  | Node { data; literal; param } ->
    Sexp.List
      [ Sexp.List
          [ Sexp.Atom "data"
          ; sexp_of_option (sexp_of_pair f Route.sexp_of_t) data
          ]
      ; Sexp.List [ Sexp.Atom "literal"; sexp_of_smap (sexp_of_t f) literal ]
      ; Sexp.List [ Sexp.Atom "param"; sexp_of_option (sexp_of_t f) param ]
      ]
...
module Tokens : sig
  ...
  let remainder t =
    let len = StringLabels.length t.s in
    if t.start >= len then
      None
    else if t.start = 0 then
      Some t.s
    else
      let res = StringLabels.sub t.s ~pos:t.start ~len:(len - t.start) in
      Some res
  ...
  let next t =
    ...
    if t.start >= len then
      ...
      match StringLabels.index_from_opt t.s t.start '/' with
      | None ->
        let res =
          let len = len - t.start in
          StringLabels.sub t.s ~pos:t.start ~len
        in
      ...
      | Some (tokens, s) ->
        ...
        | None ->
          ...
          | None ->
            None
          | Some node ->
            (loop [@tailcall]) node captured tokens)))
  in
  loop t [] tokens
...
let match_route t route =
  let rec loop t (route : Route.t) =
    ...
    | Node t ->
      (match route with
      | Route.Full_splat ->
        let here = match t.data with None -> [] | Some (a, r) -> [ a, r ] in
      ...
      | Route.Nil ->
        (match t.data with None -> [] | Some (a, r) -> [ a, r ])
      | Route.Literal (lit, route) ->
        let by_param = by_param t.param route in
        let by_literal =
  ...
  and by_param param route =
    match param with None -> [] | Some node -> loop node route
  in
  match loop t route with [] -> Ok () | routes -> Error routes
  ...
  let rec loop t (route : Route.t) =
    ...
    | Node t ->
      ...
      | Full_splat ->
        Accept (a, orig_route)
      | Nil ->
        empty_with (Some (a, orig_route))
      | Literal (lit, route) ->
        let literal =
          let node =
            Smap.find_opt lit t.literal |> Option.value ~default:empty
          in
...
let update t r ~f =
  ...
  | Error [ (a, r') ] ->
    if Route.equal r r' then
      add_no_check t r (f (Some a))
    else
      failwith "duplicate routes"
  | Ok () ->
    add_no_check t r (f None)
  | Error ([] | _ :: _ :: _) ->
    failwith "duplicate routes"

let add_route t route a =
  match match_route t route with
  | Error _ ->
    failwith "duplicate routes"
  | Ok () ->
    add_no_check t route a
